...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('enable').setDescription('Enable spam filtering.')
    )
    .addSubcommand((subcommand) =>
      subcommand.setName('disable').setDescription('Disable spam filtering.')
    )
  ...
  async execute(interaction) {
    ...
    switch (interaction.options.getSubcommand()) {
      case 'enable':
        if (rules.some((rule) => rule.name === 'Spam'))
          return await interaction.reply({ embeds: [existsEmbed] });
      ...
      case 'disable':
        ...
        if (!existingRule)
          return interaction.reply({
            embeds: [
              new EmbedBuilder()
                .setDescription('Cannot disable, rule does not exist.')
                .setColor(Colors.Red),
            ],
          });
