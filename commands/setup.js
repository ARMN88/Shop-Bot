...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((subcommand) =>
      subcommand.setName('show').setDescription('Show options for this server.')
    )
    ...
    switch (interaction.options.getSubcommandGroup()) {
      case 'set':
        switch (interaction.options.getSubcommand()) {
          case 'channel':
            const creatingChannelEmbed = new EmbedBuilder()
              .setDescription(
                `Adding <#${
                  interaction.options.getChannel('server-channel').id
                }> channel...`
              )
              .setColor(Colors.Orange);
            await interaction.reply({
              embeds: [creatingChannelEmbed],
              ephemeral: true,
            });
            const databaseChannel = await Info.findOne({
              where: {
            ...
            const createdChannelEmbed = new EmbedBuilder()
              .setDescription(
                `${Capitalize(
                  interaction.options.getString('channel-type')
                )} channel set to <#${
                  interaction.options.getChannel('server-channel').id
                }>.`
              )
            ...
            if (!databaseChannel) {
              ...
              createdChannelEmbed.setDescription(
                `${Capitalize(
                  interaction.options.getString('channel-type')
                )} channel changed to <#${
                  interaction.options.getChannel('server-channel').id
            ...
            await interaction.editReply({
              embeds: [createdChannelEmbed],
              ephemeral: true,
            });
            break;
          case 'role':
            const creatingRoleEmbed = new EmbedBuilder()
              .setDescription(
                `Adding <@&${
                  interaction.options.getRole('server-role').id
                }> role...`
              )
              .setColor(Colors.Orange);
            await interaction.reply({
              embeds: [creatingRoleEmbed],
              ephemeral: true,
            });
            const databaseRole = await Info.findOne({
              where: {
                guildId: interaction.guildId,
            ...
            const createdRoleEmbed = new EmbedBuilder()
              .setDescription(
                `${Capitalize(
                  interaction.options.getString('role-type')
                )} role set to <@&${
                  interaction.options.getRole('server-role').id
                }>.`
              )
            ...
            if (!databaseRole) {
              ...
              createdRoleEmbed.setDescription(
                `${Capitalize(
                  interaction.options.getString('role-type')
                )} role changed to <@&${
                  interaction.options.getRole('server-role').id
      ...
      case 'delete':
        switch (interaction.options.getSubcommand()) {
          case 'channel':
            const deleteChannelEmbed = new EmbedBuilder()
              .setDescription(
                `Removing ${interaction.options.getString(
                  'channel-type'
                )} channel...`
              )
              .setColor(Colors.Orange);
            await interaction.reply({
              embeds: [deleteChannelEmbed],
              ephemeral: true,
            });
            ...
            const deletedChannelEmbed = new EmbedBuilder()
              .setDescription(
                `Removed ${interaction.options.getString(
                  'channel-type'
                )} channel.`
              )
              .setColor(Colors.Green);
            await interaction.editReply({
              embeds: [deletedChannelEmbed],
              ephemeral: true,
            });
            break;
          case 'role':
            const deleteRoleEmbed = new EmbedBuilder()
              .setDescription(
                `Removing ${interaction.options.getString('role-type')} role...`
              )
              .setColor(Colors.Orange);
            await interaction.reply({
              embeds: [deleteRoleEmbed],
              ephemeral: true,
            });
            ...
            const deletedRoleEmbed = new EmbedBuilder()
              .setDescription(
                `Removed ${interaction.options.getString('role-type')} role.`
              )
              .setColor(Colors.Green);
            await interaction.editReply({
              embeds: [deletedRoleEmbed],
              ephemeral: true,
            });
            break;
        }
        break;
      default:
        const databaseRoles = await Info.findAll({
          where: {
            guildId: interaction.guildId,
            type: infoTypes.indexOf('role'),
          },
        });
        const databaseChannels = await Info.findAll({
          where: {
            guildId: interaction.guildId,
            type: infoTypes.indexOf('channel'),
          },
        ...
        databaseChannels.forEach((channel) => {
          channelString += `${Capitalize(channel.name)}: <#${
            channel.identifier
          }>\n`;
        });
